...
public class Principal {
	public static void main(String[] args) {
		...
		Pelicula peli4 = new Pelicula("Rocket League la pelicula", "Diversion", "Izan Abad Marina", 2017, 169);
		...
		Usuario user1 = new Usuario("surviv", "skiper", "mes a mes", "12/11/2017", "paypal", "C/20 C. Trinidad",
				"España", "Usuario");
		Usuario user3 = new Usuario("antonio", "rey", "mes a mes", "12/11/2017", "paypal", "C/63 C. Survivrey",
				"España", "Usuario");

		System.out.println("Bienvenido al Neclics");

		iniciarSesion(leer, user, user1, user3);
		...
		while (salirDeLaPantalla == false) {
			...
			String saltar = leer.next();
			String salirDeLaAplicacion = leer.nextLine();
			if (salirDeLaAplicacion.equalsIgnoreCase("no") || saltar.equalsIgnoreCase("no")) {
				salirDeLaPantalla = true;
			}
		}
	}
	...
	private static void iniciarSesion(Scanner leer, Usuario user, Usuario user1, Usuario user3) {
		...
		while (pasarUsuario == false) {
			...
			if (usuario.equalsIgnoreCase("-1")) {
				...
				pasarUsuario = true;
			} else if (usuario.equalsIgnoreCase(user3.getNombre())) {
				System.out.println("Usuario encontrado");	
				pasarUsuario = true;
			} else {
		...
		while (pasarContrasegna == false) {
			...
			if (contrasegna.equalsIgnoreCase(user1.getContrasegna())) {
				...
				break;
			}	else if	(contrasegna.equalsIgnoreCase(user3.getContrasegna())) {
				System.out.println("Contraseña correcta");
				break;
		    } else {
